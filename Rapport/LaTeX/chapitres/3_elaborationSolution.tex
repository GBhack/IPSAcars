		...
		\paragraph{Reconnaissance d'image}
			...
			\begin{itemize}
				\item Couleur du feu
					...
					\end{figure}
					Grâce à ce système, il nous suffit de définir expérimentalement les bornes autour de nous 3 axes.
					La fonctione \textit{inRange} est prévue à cet effet : 
					Nous déterminerons expérimentalement 



					\illu{distEval.pdf}{Schéma de principe de l'approche d'un feu}{0.35}
					Connaissant l'angle de vue de la caméra (58°), et en utilisant des fonctions géométriques simples, nous pouvons déterminer rapidement la distance minimale au feu s'il faut s'arrêter : $d_{min} = 15 cm \times tan(\frac{58}{2}) = 8,3 cm$

				\item Présence et intentions des autres utilisateurs
